...
class EMISService:
    ...
    async def iniciar_pagamento(
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            ...
            try:
                ...
                logger.info(f'Pagamento iniciado com sucesso. Transação: {d}')
                return {
                ...
                logger.error(f'Erro HTTP na EMIS: {s} - {e.response.text}')
                raise Exception(
    ...
    @staticmethod
    async def processar_callback(dados: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        t = transacao_id
        s = status
        logger.info(f'Processando callback para transação {t} - Status: {s}')
